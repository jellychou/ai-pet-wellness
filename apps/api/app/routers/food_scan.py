...
SYSTEM_PROMPT = (
    ...
    "提供營養資訊。狗和貓對很多人類食物的耐受度跟人類不一樣，有些食物對牠們是"
    "有毒的（例如巧克力、葡萄/葡萄乾、洋蔥/大蒜、木糖醇、酒精、咖啡因、夏威夷豆"
    "等），請根據你的知識謹慎判斷安全性，寧可保守也不要輕描淡寫。\n"
    "請只回傳一個 JSON 物件，格式如下，不要有其他文字：\n"
    '{"food_detected": true/false, '
    '"food_name": "食物名稱（例如「雞胸肉 Chicken Breast」），food_detected 是 '
    'false 時給空字串", '
    '"confidence": 0-100 整數, '
    '"calories": 每 100g 熱量估計（數字，kcal）, '
    '"protein": 每 100g 蛋白質估計（公克）, '
    '"fat": 每 100g 脂肪估計（公克）, '
    '"carb": 每 100g 碳水化合物估計（公克）, '
    '"fiber": 每 100g 纖維估計（公克）, '
    ...
    "設 false，其他數值/陣列欄位給 0 或空陣列，不要瞎猜。如果食物對狗或貓有毒/"
    "危險，safety_level 要低（1-2）、is_safe 設 false，suggestions 要清楚警告"
    "危險性。"
)
